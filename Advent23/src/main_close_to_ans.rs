...
fn main() {
    ...
    let count = bots
        ...
        .count();
    println!("ans1: {}", count);
    let mut current_pos = Pos(11382550, 29059445, 39808790);
    ...
    loop {
        ...
        if xp_sum < xn_sum || (xp_sum == xn_sum && xp.0.abs() < xn.0.abs()) {
            if xp_sum < current_sum || (xp_sum == current_sum && xp.0.abs() < current_pos.0.abs()) {
                current_pos.0 += step;
                current_sum = xp_sum;
            }
        } else if xn_sum < current_sum
            || (xn_sum == current_sum && xn.0.abs() < current_pos.0.abs())
        {
            current_pos.0 -= step;
            current_sum = xn_sum;
        }
        ...
        if yp_sum < yn_sum || (yp_sum == yn_sum && yp.1.abs() < yn.1.abs()) {
            if yp_sum < current_sum || (yp_sum == current_sum && yp.1.abs() < current_pos.1.abs()) {
                current_pos.1 += step;
                current_sum = yp_sum;
            }
        } else if yn_sum < current_sum
            || (yn_sum == current_sum && yn.1.abs() < current_pos.1.abs())
        {
            current_pos.1 -= step;
            current_sum = yn_sum;
        }
        ...
        if zp_sum < zn_sum || (zp_sum == zn_sum && zp.2.abs() < zn.2.abs()) {
            if zp_sum < current_sum || (zp_sum == current_sum && zp.2.abs() < current_pos.2.abs()) {
                current_pos.2 += step;
                current_sum = zp_sum;
            }
        } else if zn_sum < current_sum
            || (zn_sum == current_sum && zn.2.abs() < current_pos.2.abs())
        {
            current_pos.2 -= step;
            current_sum = zn_sum;
        }
