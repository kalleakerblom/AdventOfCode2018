...
fn main() {
    ...
    // Coordinate and equipment state combine to make up each node
    // Search graph for fastest path to target
    let start = Node(0, 0, State::Torch);
    let target = Node(target.0, target.1, State::Torch);
    if let Some(cost) = search_fastest_path(start, target, &type_map) {
        println!("ans2: {}", cost);
    } else {
...
fn search_fastest_path(
    start: Node,
    target: Node,
    type_map: &HashMap<(usize, usize), RegionType>,
) -> Option<u32> {
    let mut frontier = BinaryHeap::new();
    frontier.push(NodeOrder(start, 0));
    let mut cost_so_far = HashMap::<Node, u32>::new();
...
fn neighbors(node: &Node, map: &HashMap<(usize, usize), RegionType>) -> Vec<(Node, u32)> {
